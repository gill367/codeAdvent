...
function findNextPointandDirection(currentPoint, currentdirection, inputset) {
  ...
  switch (currentdirection) {
    case '^':
      ...
      nextDirection = input === 0 ? '<' : input === 1 ? '>' : 'UNKNOWN';
      break;
    case '<':
      nextPoint = {
        x: x,
      ...
      nextDirection = input === 0 ? 'V' : input === 1 ? '^' : 'UNKNOWN';
      break;
    case '>':
      nextPoint = {
        x: x,
      ...
      nextDirection = input === 0 ? '^' : input === 1 ? 'V' : 'UNKNOWN';
      break;
    case 'V':
      nextPoint = {
        x: input === 0 ? x + 1 : input === 1 ? x - 1 : 0,
...
async function runProgram(day2Input_changed, input, relBaseInput, startPoint, startDirection, printFlag) {
    ...
    for (var indx = 0; arr && !!arr.length && indx < arr.length;) {
      ...
      if (output.output === 0 || !!output.output) {
        ...
        if (outputPair.length === 2) {
          ...
          if(printFlag) {
          await print(pointsCovered, 80, 150, 80, 120, coloredPoint);
          }
        }
      }
      if (output.stop) {
...
function print(output, minX, maxX, minY, maxY, currentPoint){ 
  ...
  var row = "";
  var outputfiltered = outputSorted.filter((ot, indx, arr) => {
  ...
  for (var yindx = minY; yindx < maxY + 1; yindx++) {
    for (var xindx = minX; xindx < maxX + 1; xindx++) {
      var elementFound = outputfiltered.find((el) => {
        return el.x === xindx && el.y === yindx;
      });
      if(currentPoint && currentPoint.x === xindx && currentPoint.y === yindx) {
        direction = currentPoint.direction;
        row = row + (' '+ direction + ' ').bold.red;
      } else {
      var color = elementFound && elementFound.color >= 0 ? elementFound.color : -1;
      row = row + (color === 0 ? ' . '.yellow : color === -1 ? '   ' :' # '.green);
      }
    }
    row = row + '\r\n';
  } 
 ...
 // await wait(20);
    ...
    // console.log(outputfiltered.map((el) => el.x+ ", " + el.y).toString());
...
var output =  await runProgram(inputForProgram, 0, 0, {
  x: 100,
  y: 100
}, "^");
...
var finalOutput = outputSorted.filter((ot, indx, arr) => {
  var indxfound = arr.findIndex((arEl) => arEl.x === ot.x && arEl.y === ot.y);
  return indx === indxfound;
});

output = await runProgram(inputForProgram, 1, 0, {
  x: 100,
  y: 100
}, "^", true);
