...
var percentage = -1;
var intersectionPoints = withStepNo1.filter((el, indx, arr) => {
    var completedPercentange = Math.floor((indx / (arr.length) ) * 100);
    if (completedPercentange  > percentage) {
        percentage = completedPercentange;
        console.log(("..." + (completedPercentange) + " % completed..."));
    }
    var k = withStepNo2.find((a2) => a2.x === el.x && a2.y === el.y && (el.y !== 0 || el.x !== 0))
     completedPercentange = Math.floor((indx / (arr.length) ) * 100);
    if (completedPercentange  > percentage) {
        percentage = completedPercentange;
        console.log(("...--" + (completedPercentange) + " % completed..."));
    }
    ...
    console.log(("..." + (Math.floor((indx1 / (arr2.length) ) * 100)) + " % completed..."));
...
console.log("Answer to the first puzzle --> " + distance[0]); // first puzzle answer
console.log("Answer to the second puzzle -- >" + distanceinSteps[0]); // second puzzle answer
